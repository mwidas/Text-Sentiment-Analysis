...
# penalty shrinks largest coefficients towards 0
# if pure ridge mixture would be 0
lasso_spec <- logistic_reg(penalty = 0.01, mixture = 1) |> 
  set_mode("classification") |> 
  # mixed model with glm and elastic net
  set_engine("glmnet") 
...
set.seed(2024)
# running this 300 times 30 hyper-parameter values for 10 folds 
tune_rs <- tune_grid(
  tune_wf,
  incidents_folds,
  grid = lambda_grid,
...
Goal: which value of lambda leads to the best performance 
looks to be somewhere aroumd 1x10^-2
```{r plot_metrics}
...
chosen_auc <- tune_rs |> 
  # only pops up for lasso algorithm
...
looking at the words that contribute most to the likelihood of something being non-fatal
```{r words-non-fatal}
fitted_lasso |> 
  extract_fit_parsnip() |> 
  tidy() |> 
